...
class TestRouting:
    ...
    def test_matches_provider_host(self, tmp_path: Path) -> None:
        ...
        match = _route(auth, Mock(), "https", "api.openai.com", 443, "/v1/responses")
    ...
    def test_rejects_loopback_host(self, tmp_path: Path) -> None:
        ...
        assert _route(auth, Mock(), "http", "127.0.0.1", 8080, "/anything") is None
        assert _route(auth, Mock(), "http", "localhost", 8080, "/anything") is None

    def test_rejects_provider_token_endpoint(self, tmp_path: Path) -> None:
        auth = _make_auth(tmp_path)

        assert _route(auth, Mock(), "https", "github.com", 443, "/login/oauth/access_token") is None

    def test_no_match_for_unknown_host(self, tmp_path: Path) -> None:
        auth = _make_auth(tmp_path)

        assert _route(auth, Mock(), "https", "unknown.example.com", 443, "/v1") is None

    def test_no_match_without_connection(self, tmp_path: Path) -> None:
        auth = _make_auth(tmp_path)

        assert _route(auth, Mock(), "https", "api.openai.com", 443, "/v1/responses") is None
...
class TestAuthProxyAddon:
    ...
    def test_addon_injects_headers_for_matched_request(self) -> None:
        ...
        addon = AuthProxyAddon(auth=auth, audit=Mock())
...
class TestProxyRunner:
    ...
    def test_runner_sets_proxy_environment(self, tmp_path: Path) -> None:
        ...
        auth = _make_auth(tmp_path)
        runner = ProxyRunner(auth, audit=Mock())
